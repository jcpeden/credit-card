...
angular.module("creditcardApp").controller("mainController", ['$scope', function($scope) {
	...
	$scope.load = function() {
		$("[name='hide-no-bonus']").bootstrapSwitch();
		$("[name='deduct-signup-fee']").bootstrapSwitch();
	};
    ...
    $scope.bonusValueIncludes = [];

    $scope.includeCards = function(bonusValue) {
        var i = $.inArray(bonusValue, $scope.bonusValueIncludes);
        console.log( i );
        if (i > -1) {
            $scope.bonusValueIncludes.splice(i, 1);
        } else {
            $scope.bonusValueIncludes.push(bonusValue);
        }
    }
    
    $scope.bonusFilter = function(cards) {
    	
        if ($scope.bonusValueIncludes.length > 0) {

            if ($.inArray(cards.bonusValue, $scope.bonusValueIncludes) < 0)
                return;
        }
